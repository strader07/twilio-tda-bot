...
class OptionChain():
    ...
    def validate_chain(self, keyword_args = None):
        ...
        if 'strategy' in keyword_args.keys() and keyword_args['strategy'] == 'SINGLE':
            ...
            values_to_exclude = ['volatility', 'underlyingPrice', 'interestRate','daysToExpiration']
    ...
    def _get_query_parameters(self):
        ...
        new_dictionary = {key:value for key, value in self.query_parameters.items() if value != None}
    ...
    def add_chain_key(self, key_name = None, key_value = None):
        ...
        if key_name not in self.query_parameters:
            print('The key "{}" you provided is invalid for the OptionChain Object please provide on of the following valid keys: {}'.format(key_name, ', '.join(self.query_parameters.keys())))
        ...
        if key_name in self.argument_types.keys() and key_value not in self.argument_types[key_name]:
            print('The value "{}" you provided for key {} is invalid for the OptionChain Object please provide on of the following valid values: {}'.format(key_value, key_name, ', '.join(self.argument_types[key_name])))
